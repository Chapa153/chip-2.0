...
interface CrearRolViewProps {
  onBack: () => void
}

export default function CrearRolView({ onBack }: CrearRolViewProps) {
  ...
  const togglePermiso = (nodeId: string, node: PermisoNode) => {
    ...
    // Si tiene hijos, seleccionar/deseleccionar todos
    if (node.children) {
  ...
  const handleGuardarDatos = () => {
    if (!formData.codigoRol || !formData.nombreRol) {
      alert("Por favor completa los campos obligatorios (Código y Nombre del Rol)")
      return
    }
  ...
  const handleGuardarPermisos = () => {
    ...
    alert("Permisos del rol guardados correctamente")
  }
    ...
    <div className="bg-background min-h-screen p-6">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <h2 className="text-2xl font-bold text-foreground">Crear Nuevo Rol</h2>
          <p className="text-muted-foreground">
            Complete la información del rol y asigne los permisos correspondientes
          </p>
        </div>
      </div>
        ...
        <TabsContent value="permisos" className="space-y-6">
          <div className="bg-card border border-border rounded-lg p-6">
            ...
            <p className="text-sm text-muted-foreground mb-6">
              Seleccione los permisos que tendrá este rol en el sistema. Los permisos con subniveles seleccionarán
